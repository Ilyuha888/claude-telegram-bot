...
1. build a directory map
2. collect note-type statistics
3. find index notes, hub notes, and MOC pages
4. detect common link patterns
5. identify folders for inbox items, projects, people, tasks, and archives
6. infer naming conventions
7. separate user-owned notes from assistant-owned artifacts
...
Goals:
1. Identify top-level folders and their likely purpose.
...
4. Detect whether there are MOCs, hub notes, dashboards, indexes, or daily/weekly notes.
5. Detect how attachments and images are stored and linked.
6. Infer which folders are safe for assistant-generated notes and which should remain user-only.
...
- Prefer filesystem patterns, filenames, frontmatter, outbound links, and repeated structures.
...
If these zones do not exist yet, an agent may propose them:
...
Do not create them without explicit permission if the repository already has an established structure.
